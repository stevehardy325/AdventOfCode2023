...
def stepsToNextZ(start_node: str, node_dct: dict, cycle_directions: str, cycle_position: int):
    cur_node = start_node
    additional_steps = 0
...
def get_answer_optimized(inputfile):
    ...
    for i, node in enumerate(cur_nodes):
        steps_to_z_ending, _ = stepsToNextZ(node, node_dct, cycle_directions, 0)
